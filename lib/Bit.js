...
var Bit = {
    ...
    "toBinaryFormatString": Bit_toBinaryFormatString,   // Bit.toBinaryFormatString(u32:UINT32, options:Object = {}):BinaryFormatString
...
//  console.log(u32.toString(16),
//                  Bit_toBinaryFormatString(f64format[0], { comma: true }),
//                  Bit_toBinaryFormatString(f64format[1], { comma: true }),
//              exp,
//                  Bit_toBinaryFormatString(exp, { comma: true, width: 16 }));
    return (bias + 31) - exp;
}
...
function Bit_toBinaryFormatString(u32,       // @arg UINT32
                                  options) { // @arg Object - { comma, width }
                                             // @options.comma Boolean = false
                                             // @options.width UINT8 = 32
    ...
    var bin = (filler[width] + u32.toString(2)).slice(-width);

    return comma ? bin.replace(/(\d)(?=(\d\d\d\d)+(?!\d))/g, "$1,")
                 : bin;
}
