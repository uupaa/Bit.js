...
var BYTE_ORDER64  = BIG_ENDIAN ? [0, 1, 2, 3, 4, 5, 6, 7]
                               : [7, 6, 5, 4, 3, 2, 1, 0];
var BIT_MASK      = _createMaskArray();
var BIT_SHIFT     = _createShiftObject();

// --- class / interfaces ----------------------------------
var Bit = {
    "mask":         Bit_mask,       // Bit.mask(value:UINT32, bitPattern:UINT32):UINT32
    "contig":       Bit_contig,     // Bit.contig(bitPattern:UINT32):UINT8
...
// --- implements ------------------------------------------
function Bit_mask(value,        // @arg UINT32
                  bitPattern) { // @arg UINT32
                                // @ret UINT32
                                // @desc Bit shift and mask.
    var popCount = _popCount(bitPattern);

//{@dev
    if (VERIFY) {
        $valid($type(value,      "UINT32"), Bit_mask, "value");
        $valid($type(bitPattern, "UINT32"), Bit_mask, "bitPattern");
        $valid(Bit_contig(bitPattern) === popCount, Bit_mask, "bitPattern");
    }
//}@dev

    return (value >>> BIT_SHIFT[bitPattern]) & BIT_MASK[popCount];
}

function Bit_contig(bitPattern) { // @arg UINT32 - 0b0000111111110000 (bitWidth = 8)
                                  // @ret UINT8 - 8
                                  // @desc Count the number of contiguous 1 bits.
//{@dev
    if (VERIFY) {
        $valid($type(bitPattern, "UINT32"), Bit_contig, "bitPattern");
    }
//}@dev
    ...
    return (32 - Bit_nlz(bitPattern)) - Bit_ntz(bitPattern);
}
...
function _createMaskArray() { // @ret Uint32Array - [0x00, ..., 0xffffffff]
    var mask = new Uint32Array(33);

    for (var i = 0; i <= 32; ++i) {
        mask[i] = i >= 32 ? 0xffffffff
                : i <=  0 ? 0
                : ((1 << i) - 1) >>> 0;
    }
    return mask;
...
function _createShiftObject() { // @ret UINT32Object - { 0x01: 0, ..., 0x100: 8, ..., 0xffffffff: 0 }
    var i = 0, n = 0;
    var shift = {};
