...
var Bit = {
    ...
    "IEEE754":      Bit_IEEE754,    // Bit.IEEE754(num:Number, toFloat:Boolean = false):Uint32Array
...
function Bit_nlz(u32) { // @arg UINT32 - value
    ...
    //      x<<<    ntz = 3
    //
    var f64format = Bit_IEEE754(u32 === 0 ? 0.5 : u32);
...
function Bit_IEEE754(num,      // @arg Number
                     single) { // @arg Boolean = false - Single-precision
                               // @ret Uint32Array(2)
    var i;
    ...
    if (single) {
        ...
        //   31 30    23  22                    0
        //
        i = BIG_ENDIAN ? [0, 1, 2, 3] : [3, 2, 1, 0];

        FLOAT32_VIEW[0] = num;
        return (BYTE_VIEW[i[0]] << 24 | BYTE_VIEW[i[1]]  << 16 |
                BYTE_VIEW[i[2]] <<  8 | BYTE_VIEW[i[3]]) >>> 0;
    }
    ...
    //   63 62    52  51                    0
    //
    i = BIG_ENDIAN ? [0, 1, 2, 3, 4, 5, 6, 7] : [7, 6, 5, 4, 3, 2, 1, 0];

    FLOAT64_VIEW[0] = num;
    return new Uint32Array([(BYTE_VIEW[i[0]] << 24 | BYTE_VIEW[i[1]]  << 16 |
                             BYTE_VIEW[i[2]] <<  8 | BYTE_VIEW[i[3]]) >>> 0,
                            (BYTE_VIEW[i[4]] << 24 | BYTE_VIEW[i[5]]  << 16 |
                             BYTE_VIEW[i[6]] <<  8 | BYTE_VIEW[i[7]]) >>> 0]);
}
